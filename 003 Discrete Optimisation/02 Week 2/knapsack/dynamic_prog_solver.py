...
class Dynamic_solvers(object):
    ...
    def populate_dynamic_table(self, items, max_capacity):
        ...
        for item in range(1, np.shape(space)[1]):
            for capacity in range(1, np.shape(space)[0]):
                # if the weight of the current item is less than the capacity populate the value of the item
                if items[item-1].weight <= capacity:
                    space[capacity, item] = max(space[capacity, item-1], items[item-1].value + space[capacity - items[item-1].weight, item-1])            
                else:
    ...
    def traceback(self, items, space):
        """
        Creates the decision vector based on the dynamic programming search table
        Return a tuple of the maximum objective value and the decision vector
        """
        ...
        for item in range(len(items), -1, -1):
            ...
            if space[capacity, item-1] < space[capacity, item]:
                ...
                taken[item] = 1
                # reduce the used capacity by the taken item as we now look for the next best item
                capacity -= items[item-1].weight
                
        # remove the first item from the taken vector as this when selecting no items
        taken = taken[1:]
